...
def train_one_epoch(model, train_loader, criterion, optimizers, device):
    model.train()
    running_loss = 0.0
    for inputs, labels in train_loader:
        ...
        # Apply updates for each optimizer and collect updated weights
        for optimizer in optimizers:
            optimizer.step()
        ...
        for key in pre_update_weights.keys():
            final_weights[key] = sum([optimizer_updates[i][key] for i in range(len(optimizers))]) / len(optimizers)
...
# Function to initialize model, criterion, and multiple optimizers
def initialize_model(input_size, num_classes, optimizer_configs, hl_sizes, use_bn, dropout_rate, device):
    ...
    return model, criterion, optimizers



# Function to run K-Fold Cross-Validation with Early Stopping and multiple optimizers
def run_k_fold_cv(almonds_train, num_epochs, batch_size, optimizer_configs, hl_sizes, use_bn, dropout_rate, k_folds, device, patience=15):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(almonds_train)):
        ...
        model, criterion, optimizers = initialize_model(almonds_train.num_features(), almonds_train.num_classes(),
                                                        optimizer_configs, hl_sizes, use_bn, dropout_rate, device)
        best_val_accuracy = 0.0
        epochs_no_improve = 0  # To track early stopping

        for epoch in range(num_epochs):
            loss = train_one_epoch(model, train_loader, criterion, optimizers, device)
...
def objective(params):
    ...
    avg_val_accuracy = run_k_fold_cv(almonds_train, params['epochs'], params['batch_size'], params['optimizers'],
...
def main():
   ...
   # Updated PyHopper search space to support multiple optimizers
    search = pyhopper.Search({
        'epochs': pyhopper.choice(200),
        'batch_size': pyhopper.choice(64, 128),
        'optimizers': pyhopper.choice(  # Support multiple optimizers
        ...
        'dropout_rate': pyhopper.choice(0.0)  # Dropout rate
    })
